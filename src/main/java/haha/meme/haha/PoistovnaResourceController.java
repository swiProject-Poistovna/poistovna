...
public class PoistovnaResourceController {
	...
	public List<PoistovnaDataOut> dajVyhovujucePoistovne(@RequestBody PoistenecDataIn poistenecData) {
		// this.poistovnaDataOutList =
		// poistovneDBService.dajVyhovujucePoistovneMimoDB(poistenecData);
		this.poistovnaDataOutList = poistovneDBService.dajVyhovujucePoistovne(poistenecData);
		if (this.poistovnaDataOutList == null || this.poistovnaDataOutList.size() == 0) {
			throw new PoistovneNotFoundException();
		}
